...
function [e,c_L,c_Di,A_matrix] = PLLT(b,a0_t,a0_r,c_t,c_r,aero_t,aero_r,geo_t,geo_r,N)
    ...
    for i = 1:N
        for j = 1:N
            Mij(i,j) = ( (4*b)./ (a0(i).*c(i)) ) .* sin(((2.*j) - 1).*theta(i)) + ((2*j)-1)*((sin(((2.*j) - 1).*theta(i)))./sin(theta(i)));
        end
    end
    A_matrix = Mij\b_i;
    ...
    %could be an error from here down because c_Di and e don't match the
    %values in the annocment
    C_Di_total = 0;
    for j=1:N
        delta(j)= C_Di_total;
        C_Di_total = C_Di_total + (2*j-1)*((A_matrix(j)/A_matrix(1))^2);
